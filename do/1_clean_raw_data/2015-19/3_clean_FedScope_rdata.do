...
save "$wdata/FedScope.dta", replace

*********************************** Level 2 ***********************************

use "$wdata/FedScope.dta", clear

joinby unit_fedscope using "$wdata/code_transfer.dta"
order unit_fevs, a(unit_fedscope)

collapse (mean) age eduyr salary los Professional Administrative Technical Clerical White_Collar Blue_Collar fulltime [aweight = employee], by(year agency unit_fevs SS)

reshape wide age eduyr salary los Professional Administrative Technical Clerical White_Collar Blue_Collar fulltime, i(year agency unit_fevs) j(SS)

foreach x in age eduyr salary los Professional Administrative Technical Clerical White_Collar Blue_Collar fulltime{
	rename `x'0 `x'_sub
	rename `x'1 `x'_sup
}
...
save "$wdata/FedScope_level2_OO.dta", replace
...
replace unit_fevs = agency + "ZZ" if unit_fevs == ""
...
save "$wdata/FedScope_level2_ZZ.dta", replace

use "$wdata/FedScope_level2_OO.dta",clear
ap using "$wdata/FedScope_level2_ZZ.dta"

sort year agency unit
duplicates drop

drop if unit == ""
save "$wdata/FedScope_level2.dta", replace
...
merge 1:m agency using "$wdata/FedScope.dta"
...
collapse (mean) age eduyr salary los Professional Administrative Technical Clerical White_Collar Blue_Collar fulltime [aweight = employee], by(year agency SS)

reshape wide age eduyr salary los Professional Administrative Technical Clerical White_Collar Blue_Collar fulltime, i(year agency) j(SS)
...
order year agency
save "$wdata/FedScope_level1.dta", replace
