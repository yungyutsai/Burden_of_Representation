...
save "$wdata/FedScope_2023_Race.dta", replace
...
order year agency
save "$wdata/FedScope_Race_2023_level1.dta", replace
...
order year agency unit level race Number SSTot
keep year agency unit level race Number SSTot
save "$wdata/FedScope_2023_Race_All.dta", replace
...
merge 1:m agency using "$wdata/FedScope_2023_Race_All.dta"
keep if _m == 3
keep if level == 1
drop _m

collapse (mean)Number SSTot, by(year agency level race)

replace Number = round(Number,1)

sort year agency race

** Calculate the maxmium number of missing value
forv i = 3(1)8{
	gen v0 = Number if inrange(race,3,8)
	egen nonmissing = sum(v0), by(year agency level) //Total minority number for non-missing
	gen v1 = Number if race == 2
	egen minority = sum(v1), by(year agency level) //Total minority
	gen maxmium = minority - nonmissing //maxmium number of missing value
	replace maxmium = 0 if maxmium < 0 //replace negative to 0
	replace maxmium = round(maxmium,1) //round to integer
	set seed `i'
...
collapse (sum)Number SSTot, by(year agency level race)

gen prop = Number / SSTot

keep year agency race prop

drop if race == 0
reshape wide prop*, i(year agency) j(race)

rename prop1 prop_white
rename prop2 prop_minority
rename prop3 prop_black
rename prop4 prop_hisp
rename prop5 prop_asian
rename prop6 prop_aian
rename prop7 prop_nhpi
rename prop8 prop_twomore
rename prop9 prop_unspecified

order year agency
save "$wdata/FedScope_Race_2023_level1_All.dta", replace
