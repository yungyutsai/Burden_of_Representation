...
cap rm "$tab/TabA8.xls"
cap rm "$tab/TabA8.txt"

foreach y in sat rc sr{
	
	reg `y' prop_sub_samerace prop_sup_samerace $cov1 $cov2 $cov3 i.agencyrace i.year [aw=weight1], cl(agencyyear) 
	outreg2 using "$tab/TabA8.xls", append dec(3) keep(prop_sub_samerace)

	reg `y' c.prop_sub_samerace#i.minor prop_sup_samerace $cov1 $cov2 $cov3 i.agencyrace i.year [aw=weight1], cl(agencyyear)
	outreg2 using "$tab/TabA8.xls", append dec(3) keep(c.prop_sub_samerace#i.minor)
	
	reg `y' c.prop_sub_samerace#i.race prop_sup_samerace $cov1 $cov2 $cov3 i.agencyrace i.year [aw=weight1], cl(agencyyear)
	outreg2 using "$tab/TabA8.xls", append dec(3) keep(c.prop_sub_samerace#i.race)
	
	
	reg `y' c.prop_sub_samerace##c.prop_sup_samerace $cov1 $cov2 $cov3 i.agencyrace i.year [aw=weight1], cl(agencyyear)
	outreg2 using "$tab/TabA8.xls", append dec(3) keep(c.prop_sub_samerace##c.prop_sup_samerace prop_sup_samerace)
