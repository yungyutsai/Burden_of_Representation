...
import delimited using "$tab/FigA1.txt", clear
...
twoway 	(sc var b if var > 6, mc(navy)) ///
		...
		yscale(ra(7.5 17)) ///
		xtitle("Effect of % Same-race Subordinates") ///
		ytitle("") legend(order(5 "White" 6 "non-White")) ///
		ylabel(	11 "Support employee development" ///
				12 "Listens to me" ///
				13 "Treats me with respect" ///
				14 "Trust and confidence" ///
				15 "Immediate supervisor" ///
				16 "Senior leaders" ///
				8 "Expectation of job" ///
				9 "Agency goals", angle(0) labsize(small)) ///
		text(10 -2.1 "Role Clarity", place(w) just(right)) ///
		text(17 -2.1 "Supervisor Relationship", place(w) just(right))
		
graph export "$fig/FigA1_Med.png", as(png) replace


***************************


import delimited using "$tab/FigA2.txt", clear
...
twoway 	(sc placey1 b1 if var > 6, mc(navy)) ///
		...
		(connect placey1 b1 if var ==0, mc(maroon) ms(Sh)), ///
		scheme(s1color) ///
		xlabel(-6(2)6, format(%4.0f)) xline(0, lc(black) lp(dash))  ///
		ylabel(none) yscale(ra(7.5 17)) ///
		xtitle("Effect of % Same-race Subordinates") ///
		ytitle("") legend(order(5 "White" 6 "non-White")) ///
		ylabel(	11 "Support employee development" ///
				12 "Listens to me" ///
				...
				8 "Expectation of job" ///
				9 "Agency goals", angle(0) labsize(small)) ///
		text(10 -6.8 "Role Clarity", place(w) just(right)) ///
		text(17 -6.8 "Supervisor Relationship", place(w) just(right))
graph export "$fig/FigA2_Med.png", as(png) replace
